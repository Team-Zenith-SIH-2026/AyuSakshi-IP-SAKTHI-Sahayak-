...
export const Sidebar = () => {
  ...
  return (
    <>
      ...
      {/* Responsive Sidebar Container: Off-canvas drawer on mobile/tablet, in-flow sidebar on desktop */}
      <aside
        className={`
          fixed lg:static inset-y-0 left-0 z-50 lg:z-auto
          w-72 sm:w-80 lg:w-64 xl:w-72 flex-shrink-0
          border-r border-slate-200 dark:border-darkbg-border bg-white dark:bg-darkbg-950
          flex flex-col justify-between h-full lg:h-[calc(100vh-3.5rem)] sm:lg:h-[calc(100vh-4rem)] select-none
          transition-transform duration-300 ease-in-out
          ${isSidebarOpen ? 'translate-x-0 shadow-2xl lg:shadow-none' : '-translate-x-full lg:translate-x-0'}
        `}
      >
        {/* Mobile Header with Close Button */}
        <div className="flex lg:hidden items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-darkbg-border">
          <div className="flex items-center space-x-2">
            <Sparkles className="w-4 h-4 text-emerald-500" />
            <span className="font-bold text-xs text-slate-800 dark:text-slate-200">AYUSH IP Navigator</span>
          </div>
          <button
            type="button"
            onClick={closeSidebar}
